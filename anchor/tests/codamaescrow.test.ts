...
describe("Escrow", () => {
  ...
  // Same as declare_id in the rust program
  programId = CODAMAESCROW_PROGRAM_ADDRESS;
  ...
  before(async () => {
    ...
    console.log(`
      ...
      bobTokenAccountA: ${bobTokenAccountA},
      aliceTokenAccountB: ${aliceTokenAccountB},
    `);

    let a = await connection.getTokenAccountBalance(
      {
  ...
  describe("can get all the offers", () => {
    test("successfully gets all the offers", async () => {
      const getOffers = connection.getAccountsFactory(
        programClient.CODAMAESCROW_PROGRAM_ADDRESS,
        OFFER_DISCRIMINATOR,
        getOfferDecoder(),
      );
